...
class Presence:
    ...
    def _get_strategy(self):
        ...
        elif self.model_tag == "rembg":
            from ..engine_models.bg_remover import BgRemover
            return BgRemover(backend=self.backend)
        else:
    ...
    async def remove_background(self, payload: Dict[str, Any]):
        ...
        if not isinstance(self._strategy, BgRemover):
            raise ValueError("this is not the bgremover strategy instance")
        
        image_data = payload.get("image")

        if not image_data:
            raise ValueError("Eyes.remove_background: Missing 'image' in payload")

        pil_img = self.decode_image(image_data)
        
        output_img = self._strategy.run(pil_img)
