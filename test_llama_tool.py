...
class ModelMetadata:
    ...
    def _resolve(self):
        ...
        if repo_id and filename:
            ...
            if not self.file_size_gb:
                self.file_size_gb = self._get_remote_size(url)

            self._raw_metadata = self._scan_remote(url)
            
    def _scan_remote(self, url: str) -> dict:
        """
        ...
        try:
            with httpx.Client(follow_redirects=True, timeout=600) as client:
                ...
                response = client.get(url, headers=range_header)
    ...
    def _get_remote_size(self, url: str) -> float:
        try:
            with httpx.Client(follow_redirects=True, timeout=600) as client:
                res = client.head(url)
...
if __name__ == "__main__":
    ...
    metadata = ModelMetadata("", repo_id=args.repo_id, filename=args.filename)
